...
# ----- ЕКРАН: НОВА ПРОДАЖБА / ПОС (Модул 4) -----
    ...
    if st.session_state.sale_cart:
        ...
        if critical:
            # Екранираме заглавията — идват от свободен текст, а тук рендираме HTML.
            titles = html.escape(", ".join(critical))
            st.markdown(
                ...
                with st.container(border=True):
                    ...
                    with cols[5]:
                        if st.button("Ръчно одобрение",
                                     ...
                                     help="Приеми разликата, запиши я като разход "
                                          "и маркирай поръчката платена"):
                            db.set_sale_status(m["sale_id"], "Платена")
                            # Разликата = очаквано (Bookspace) − получено (куриер).
                            # Положителна = получили сме по-малко → разход; при
                            # надплащане е отрицателна (действа като приход).
                            gap = round(m["bookspace"] - m["speedy"], 2)
                            db.add_expense(
                                str(date.today()), "Куриерски разлики",
                                f"Товарителница {m['waybill']} · поръчка "
                                f"№{m['order_number']} "
                                f"(Bookspace {m['bookspace']:.2f} / куриер "
                                f"{m['speedy']:.2f})",
                                gap, document_number=m["waybill"])
                            st.success(
                                f"Поръчка №{m['order_number']} — одобрена, "
                                f"платена. Разлика {gap:.2f} лв. записана като "
                                "разход в категория Куриерски разлики.")
                            st.rerun()
    ...
    with tab_list:
        ...
        with f3:
            cat_filter = st.selectbox("Категория", [
                ...
                "Битови сметки/Ток/Интернет", "Куриерски разлики", "Други",
            ])
